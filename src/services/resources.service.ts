...
export class ResourcesService {
  ...
  public determineLocation(): Promise<any> {
    const client: HttpUtilities = new HttpUtilities();

    return new Promise((resolve, reject) => {
      if (this.configService.get("geoLocation")) {
        if (
          !isNaN(this.configService.get("geoLocation.latitude")) &&
          !isNaN(this.configService.get("geoLocation.longitude"))
        ) {
          resolve({
            latitude: this.configService.get("geoLocation.latitude"),
            longitude: this.configService.get("geoLocation.longitude"),
          });
        }
      }
      ...
      client
        ...
        .then((jsonData) => {
          ...
          if (parsed.images && parsed.images.length > 0) {
            const img = "https://www.bing.com" + parsed.images[0].url;
            this.logger.debug("Bing Image url", img);
            resolve(img);
          } else {
            this.logger.warn("No bing image");
            resolve("");
          }
        })
        .catch((err) => {
          this.logger.error("Error getting Bing image", err);
          reject(err);
        });
    });
  }
