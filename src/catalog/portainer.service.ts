...
export class PortainerService {
  ...
  private fetchCategories(id: string): Promise<string[]> {
    return new Promise<string[]>((resolve, reject) => {
      this.templateModel
        .find({ catalogId: { $eq: id } }, { categories: 1 })
        .lean()
        .exec()
        .then((res) => {
          let categories = [];
          res.forEach((r) => {
            const rs = r.categories.filter((n) => {
              return !categories.includes(n);
            });
            categories = categories.concat(rs);
          });
          resolve(categories.sort());
        })
        .catch((err) => {
          this.logger.error("Error in fetchCategories", err);
          reject(err);
        });
    });
  }
  ...
  public toDockerRun(cfg: TemplateCreateRequest): TemplateCreateResponse {
    try {
      ...
      if (t.network != "") {
        sb.appendFormat("--network {0}", t.network);
      }
